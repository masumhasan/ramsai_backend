...
export class SubscriptionController {
  ...
  public static async handleRevenueCatWebhook(req: Request, res: Response) {
    try {
      const authHeader = req.headers.authorization;
      const expectedBearer = config.rcBearer;
      ...
      let subscriptionStatus: 'inactive' | 'active' | 'trial' | 'expired' = 'inactive';
      let currentPlan: 'basic' | 'premium' = 'basic';
      ...
      try {
        ...
        const rcResponse = await axios.get(
          ...
          {
            headers: {
              Authorization: `Bearer ${config.rcApiKey}`,
      ...
      if (subscriptionProductId) {
        const matchedPlan = await SubscriptionPlan.findOne({ type: currentPlan });
        if (matchedPlan) {
