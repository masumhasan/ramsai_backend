...
import { PRODUCT_ANALYSIS_SYSTEM_PROMPT, getProductBarcodeUserPrompt, getProductLabelUserPrompt } from '../prompts/product.prompts';

export class ProductService extends AIService {
  
  // Mock products matching Stitch design screens for reliable testing and demos
  private static MOCK_PRODUCTS: Record<string, any> = {
   ...
   */
  public static async analyzeProductBarcode(barcode: string, language: string = 'en'): Promise<any> {
    ...
    try {
      ...
      const response = await axios.get(offUrl, { timeout: 8000 });
      ...
      if (response.data && response.data.status === 1 && response.data.product) {
        ...
        const messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[] = [
          { role: 'system', content: PRODUCT_ANALYSIS_SYSTEM_PROMPT(language) },
          { role: 'user', content: getProductBarcodeUserPrompt(productName, category, ingredientsText, language) }
        ];
   ...
   */
  public static async analyzeProductLabelImage(imageBuffer: Buffer, mimeType: string, language: string = 'en'): Promise<any> {
    ...
    const messages: OpenAI.Chat.Completions.ChatCompletionMessageParam[] = [
      ...
      { 
        role: 'user', 
        content: [
          { type: 'text', text: getProductLabelUserPrompt(language) },
   ...
   */
  private static async translateMockIfNeeded(mockProduct: any, language: string): Promise<any> {
    if (language === 'en') {
      return mockProduct;
    }
