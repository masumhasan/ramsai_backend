export interface WorkoutPlanAccessResult {
  allowed: boolean;
  isPremium: boolean;
  isTrial: boolean;
...
export interface UserSubscriptionContext {
  ...
  workoutTrialExpiresAt?: Date | string;
}
 ...
 */
export function checkWorkoutPlanAccess(user: UserSubscriptionContext | null | undefined): WorkoutPlanAccessResult {
  if (!user) {
    return {
      allowed: false,
      isPremium: false,
